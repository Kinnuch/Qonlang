...
function makeAelith(): void {
  ...
  about.markdown = [
    ...
    '- **语料**：已 gloss 并确认的例句、其他正字法、手填的文字写法、自由行、出处与标签；dovar 靠「连读浊化」反推认出；ilenkasoda 是两个词连写再带格缀，没写分隔符也切得开；人名 Mira 故意没进词库，悬浮时是「没有找到」；原文里的句号、问号照原样显示，不算词；新增一句例句，点译文旁的「工作台」写「我知道你的朋友在房子里」，下面会浮出 men、bil-、sen、tovar、kaso 这些词，拖上去排好就能加进语料',
    '- **短语**：分类、变体、发音、方括号占位符',
