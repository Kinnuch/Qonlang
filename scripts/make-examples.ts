...
function makeAelith(): void {
  ...
  // 收藏（列表左边的星星）与记号（单词前面加 *）：各挑几个词
  for (const lemma of ['kaso', 'nöl', 'sila']) lex.get(lemma)!.favorite = true
  for (const lemma of ['kaso', 'kel-']) lex.get(lemma)!.marked = true
  ...
  const spokenRs = createRuleSet(
    ...
    [
      ...
      '[+鼻音] = m n ng',
      'a > ã / _[+鼻音]σ',
      '; 两个音并成一个：元音之间的 t、d 都闪成 r。右边的 r 照写两遍，按位置一一对应',
      '{齿塞}=t d',
      '{闪音}=r r',
      '{齿塞} > {闪音} / V_V',
      '-* 口语'
  ...
  spokenRs.notes =
    '测试台的每一列都带着 ˈ：到重音规则那一行时标上，之后的规则照常跨过它匹配。敲的词对上词库里勾了「对重音影响」的词条时带上它的词类与特殊重音：biz、sen 是代词，不重读，e 也跟着弱化；telikaso 重读 ka。kasoda 里元音之间的 d 闪成 r。'
  ...
  // 「微调」里现写一对音类：写在这一步的定义对这一步的每条规则都有效
  ...
  sandhiP.generators[value(sandhi, 'VOI')] = pipeline(
    ...
    step('adjust', {
      text: [
        '; 两个音类在这一步里现写，按位置一一对应：p → b、t → d、k → g',
        '{清塞}=p t k',
        '{浊塞}=b d g',
        '{清塞} > {浊塞} / #_'
      ].join('\n')
  ...
  about.markdown = [
    ...
    '- **音变**：四套规则集（元音和谐、Proto → Aelith、Proto → Merun、书面语 → 口语），阶段绑定语言，测试台词表；「Proto → Aelith」里有满足 / 不满足环境两路的规则（`θ > t?s / #_`：词首变 t、别处变 s）和带两个排除的规则（`u > o / _# - k_ , g_`），整库演化推出来的正是词库里的 kaso、nöl、sör-；「书面语 → 口语」演示特征（`[+浊] = b d g z v`，规则里写 `[+浊] > [-浊] / _σ`）、重音规则（`ˈ = @ , <代词|小品词> 0 , 1`，测试台每一列都带着 ˈ；biz、sen 是代词不重读，telikaso 重读 ka）、音节边界 σ（不重读的 e 弱化：`e > ə / σ(C)(C)_ - ˈ(C)(C)_`，sen → sən）和两个音并成一个（`{齿塞}=t d` 对 `{闪音}=r r`：元音之间的 t、d 都闪成 r，kasoda → ˈkasora；右边的 r 照写两遍，按位置一一对应）',
    '- **语素**：词根 / 前缀 / 后缀 / 中缀 / 环缀 / 附着词 / 小品词，异体形环境，词源；「对重音影响」：附着词 =mU 不重读，前缀 be- 算作形容词',
    '- **词库**：显示模式里 kaso、nöl 这些从祖语来的词，例句上方有一行历史形式（PAe kasu → CAe kaso → Ae kaso）；多义项、一个义项几个语域（dünar）、方言、标签、维度、复合词类与义项自己的词类（kara）、词干槽、词源链（词根 / 复合 / 派生 / 音变 / 借词 / 自己写的类别「仿译」）、自定义关系种类（押韵）、配图、手改发音、「对重音影响」（代词与小品词传递词性、telikaso 传递特殊重音）；sörmek 是从 sör- 的动名词「生成到词库」的，词源与关系都是自动填的；vesa 故意没写释义，词库里标红，底栏右边的问题统计点开能跳过去；kaso、nöl、sila 收了藏（列表左边的星星，顶上的星星按钮只看这几个），kaso 与 kel- 打了记号，单词前面带一个 *（符号在设置 → 基本信息里改）；词条卡里的屈折形写维度的 gloss 缩写（SG.NOM），鼠标停上去看全称',
    ...
    '- **构形**：流水线的八种步骤（前缀、后缀、中缀、环缀、音变、模板、重叠、微调）、变体（基础那套改名叫「书面」）、继承、屏蔽槽位、手填表、作用于所有词的「连读浊化」（ve 后面 tovar → dovar；「微调」这一步里现写了一对音类 `{清塞}=p t k`、`{浊塞}=b d g`，按位置对应）、一个词类绑几个构形（「动词」默认变位法一，tur-、sal- 在词条里挑了变位法二，ol- 用不规则）、按条件换字母（名词与格只写一条 ¢{阴:g|k}A：阴性的 sila、vene 是 silaga、venege，其余是 kasoka 这样）、构形套构形（「动名词」加 -mAk 之后套进「名词」的格：sörmek、sörmekde）、一个词条几个构形（sör- 既变位又有动名词）、槽位继承（名词复数的宾格、位格从「复数.主格」kasolar 接着加格后缀）、按维度取值挑异体形（复数标记在与格前是 -lAn：kasolanka）、简洁模式的槽位继承（动词第三人称那几格不写，接着「极性-时」两个维度的槽位往下变）、影响发音（位格另写了发音流水线，元音之间的 d 读 ð：kasoda 读 ˈkasoða）、复制粘贴槽位和整套变体、双击槽位名把写错的写法挪到别的槽位；页签按词类分组；测试台切到「自由」随便写一个形式看它变成什么',
    '- **语料**：已 gloss 并确认的例句、其他正字法、手填的文字写法、自由行、出处与标签；dovar 靠「连读浊化」反推认出；ilenkasoda 是两个词连写再带格缀，没写分隔符也切得开；人名 Mira 故意没进词库，悬浮时是「没有找到」；原文里的句号、问号照原样显示，不算词；新增一句例句，点译文旁的「工作台」写「我知道你的朋友在房子里」，下面会浮出 men、bil-、sen、tovar、kaso 这些词，拖上去排好就能加进语料；点上面的某个词，附着台里挑数和格（kaso 挑位格就是 kasoda），泡泡里的「-(U)n 你的」拖到 tovar 上就贴成 tovarun；译文里的「在」「里」会提示没有找到对应的词（Aelith 用位格表达，不用新建）；在「词库中没有？直接输入」里写一个新词，挑它算作名词，就能按名词的构形变格',
    '- **短语**：分类、变体、发音、方括号占位符',
    '- **文档**：项目级与语言级页面，写 `[[kaso]]` 点到词库里的词；「Aelith 语法概要」那一页还写了 `[[语素:-lAr]]`、`[[音变:Proto → Aelith#现代语]]`、`[[构形:名词#单数.位格]]`——带前缀能链到语素、语言、文字、音变的某个阶段、构形的某一格（前缀中英文都认，工具条上的「插入链接」可以挑）',
    '- **设置**：导出模板、缩写表',
    '',
  ...
  grammar.markdown = [
    ...
    '- 从祖语来的那条链：[[音变:Proto → Aelith#现代语]]；刻文写法见 [[文字:Aelith 刻文]]，姊妹语见 [[语言:Merun]]。',
    '',
    '## 名词',
    '词根-数-格：复数 [[语素:-lAr]]，宾格 [[morpheme:-(U)m]]，位格 [[语素:-dA]]，与格 [[语素:-kA]]；阴性名词（[[sila]]、[[vene]]）的与格是 -gA，构形里只写一条 ¢{阴:g|k}A。整套变格见 [[构形:名词]]，单数位格那一格是 [[构形:名词#单数.位格|名词 · 单数位格]]。',
    '',
    '| | 单数 | 复数 |',
    '|---|---|---|',
    '| 主格 | [[kaso]] | kasolar |',
...
function makeTsahun(): void {
  ...
  about.markdown = [
    ...
    '- **语料**：重叠形也能被自动 gloss 认出来（`lun35lun35`），带空格的 `ngo21 tui55` 并成一个词认；`hok33 lo21` 里的同形词故意没确认，悬浮时并排给候选；工作台里点一个动词，附着台列出语料里跟动词用过的小品词：完成体 ta33 放在后面、否定 mo35 放在前面（按语料里惯常挨着的那一边）',
