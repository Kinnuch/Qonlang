...
import { dictionaryHtml, dictionaryMarkdown, renderEntries, collectEntries } from '$lib/export/dictionary'
import type { Project } from '$lib/core/model'

describe('markdown', () => {
  it('renders headings, lists, tables, code and wiki links', () => {
    const html = mdToHtml('# T\n\n- a\n- **b**\n\n| x | y |\n| --- | --- |\n| 1 | `2` |\n\n```\ncode <x>\n```\n\n> q\n\n[[kaso]] [[zzz|Z]] [site](https://a.b)', { resolve: (n) => (n === 'kaso' ? 'id1' : null) })
    expect(html).toContain('<h2>T</h2>')
    expect(html).toContain('<ul><li>a</li><li><strong>b</strong></li></ul>')
    expect(html).toContain('<table><thead><tr><th>x</th><th>y</th></tr></thead><tbody><tr><td>1</td><td><code>2</code></td></tr></tbody></table>')
...
describe('evolution', () => {
  function setup() {
    ...
    const project = parseProject(JSON.stringify({ schemaVersion: 1, meta: { name: 'x' }, languages: [proto, daughter], ruleSets: [rs] })) as Project
  ...
  it('plans, creates with etymology, then updates instead of duplicating', () => {
    ...
    const r1 = applyEvolution(project, rows, o, { copySenses: true, updateExisting: true, createOnCollision: false })
    ...
    const r2 = applyEvolution(project, rows2, { ...o, program: program2 }, { copySenses: true, updateExisting: true, createOnCollision: false })
  ...
  it('respects stage bounds', () => {
    ...
    const rows = planEvolution(project, { ruleSet: rs, program, sourceLanguageId: proto.id, targetLanguageId: daughter.id, stopAt: 'P' })
...
describe('dictionary export', () => {
  ...
  const o = { title: 'Aelith', glossLangs: ['zh', 'en'], includeForms: true, includeEtymology: true, includeScript: true, includeNotes: false, groupByInitial: true }
