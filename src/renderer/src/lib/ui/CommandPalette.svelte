<script lang="ts">
    ...
    const acts: Item[] = [
      {
        kind: 'cmd',
        id: 'save',
        title: t('palette.save'),
        sub: 'Ctrl+S',
        section: ui.section,
        run: () => void projectState.save()
      },
      {
        kind: 'cmd',
        id: 'saveAs',
        title: t('palette.saveAs'),
        sub: 'Ctrl+Shift+S',
        section: ui.section,
        run: () => void projectState.save(true)
      },
      {
        kind: 'cmd',
        id: 'chars',
      ...
      },
      {
        ...
        sub: 'Ctrl+\\',
        section: ui.section,
        run: () => (ui.inspectorOpen = !ui.inspectorOpen)
      },
      {
        kind: 'cmd',
        id: 'newLexeme',
  ...
  const results = $derived.by((): Item[] => {
    ...
    const langName = (id: string | null): string =>
      project.languages.find((l) => l.id === id)?.name ?? ''
    for (const l of project.lexemes) {
      const def = l.senses
        .map((s) => pickText(s.definition, glossLangs))
        .filter(Boolean)
        .join('; ')
      if (hit(l.lemma) || hit(def))
        out.push({
          kind: 'lexeme',
          id: l.id,
          title: l.lemma,
          sub: `${t('nav.lexicon')} · ${langName(l.languageId)} · ${def}`,
          section: 'lexicon'
        })
    ...
    for (const m of project.morphemes) {
      ...
      if (hit(m.form) || hit(m.gloss) || hit(meaning))
        out.push({
          kind: 'morpheme',
          id: m.id,
          title: m.form,
          sub: `${t('nav.morphemes')} · ${m.gloss || meaning}`,
          section: 'morphemes'
        })
      if (out.length > 90) break
    }
    for (const s of project.sentences)
      if (hit(s.text) || Object.values(s.translation).some(hit))
        out.push({
          kind: 'sentence',
          id: s.id,
          title: s.text,
          sub: `${t('nav.corpus')} · ${pickText(s.translation, glossLangs)}`,
          section: 'corpus'
        })
    for (const p of project.phrasebook)
      if (hit(p.text) || Object.values(p.translation).some(hit))
        out.push({
          kind: 'phrase',
          id: p.id,
          title: p.text,
          sub: `${t('nav.phrasebook')} · ${pickText(p.translation, glossLangs)}`,
          section: 'phrasebook'
        })
    for (const d of project.docs)
      if (hit(d.title) || hit(d.markdown))
        out.push({ kind: 'doc', id: d.id, title: d.title, sub: t('nav.docs'), section: 'docs' })
    for (const r of project.ruleSets)
      if (hit(r.name))
        out.push({
          ...
          id: r.id,
          title: r.name,
          sub: t('nav.soundChanges'),
          section: 'soundChanges'
        })
  ...
  function pick(it: Item): void {
    ...
    if (
      ...
      const lang =
        it.kind === 'lexeme'
          ? project?.lexemes.find((x) => x.id === it.id)?.languageId
          : it.kind === 'sentence'
            ? project?.sentences.find((x) => x.id === it.id)?.languageId
            : it.kind === 'phrase'
        ...
        <li>
          ...
          >
            ...
            <span class="grow ellip"
              ><span
                class="ttl"
                class:data={['lexeme', 'morpheme', 'sentence', 'phrase'].includes(it.kind)}
                >{it.title || '—'}</span
              ><span class="sub">{it.sub}</span></span
            >
