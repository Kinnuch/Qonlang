export default {
  ...
  paradigms: {
    ...
    newParadigm: '新建范式',
    untitled: '未命名范式',
    empty: '还没有范式。新建一个，选维度、给每个槽位配生成器，再绑定到词类。',
    dimensions: '维度',
  ...
  images: {
    ...
    crop: '裁剪',
    zoom: '缩放',
    cropHint: '这张图是 {iw}×{ih}，与项目设置的配图尺寸 {w}×{h} 不符：拖动亮框选出要保留的区域（滚轮或滑块缩放），或先到「设置 → 词条配图尺寸」改默认尺寸再导入。',
    sizeHint: '统一尺寸 {w}×{h}；尺寸相符的图片原样保存，比例相同的等比缩放，其余会弹出裁剪框。PNG / GIF 无损保存，JPEG / WebP 沿用原格式高质量编码。'
  },
  skin: {
    title: '皮肤',
