...
export interface LexcFile {
  ...
  Phonotactics?: Record<
    string,
    { Onsets: string; Medials: string; Codas: string; Vowels: string; Illegals: string }
  >
  Lects?: string[]
...
export function parseLexc(text: string): LexcFile {
  ...
  if (!obj || typeof obj !== 'object' || !('Lexicon' in obj))
    throw new Error('不是 Lexicanter 文件')
  return obj
}

/** 新建一个项目并导入 */
export function lexicanterToProject(
  file: LexcFile,
  opts: LexicanterImportOptions
): { project: Project; report: LexicanterReport } {
  const project = createProject({
    name: file.Name || 'Lexicanter',
    template: 'lexicanter',
...
export function mergeLexicanter(
  project: Project,
  file: LexcFile,
  opts: LexicanterImportOptions
): LexicanterReport {
  const report: LexicanterReport = {
    languages: [],
    lexemes: 0,
    phrases: 0,
    ruleSets: 0,
  ...
  for (const [parentWord, ety] of Object.entries(file.Etymologies ?? {})) {
    const parentId =
      ety.source === THIS ? idByWord.get(parentWord) : relativeMaps.get(ety.source)?.get(parentWord)
    for (const d of ety.descendants ?? []) {
      const childId =
        d.source === THIS ? idByWord.get(d.name) : relativeMaps.get(d.source)?.get(d.name)
      ...
      if (parentId) child.etymology.sources.push({ kind: 'lexeme', id: parentId })
        ...
        child.etymology.sources.push({
          kind: 'external',
          language: ety.source === THIS ? lang.name : ety.source,
          form: parentWord,
          meaning: ''
        })
  ...
  for (const [category, phrases] of Object.entries(file.Phrasebook ?? {})) {
    for (const [text, p] of Object.entries(phrases)) {
      const ph: Phrase = {
        ...
        variants: Object.entries(p.variants ?? {}).map(([v, d]) => ({
          text: v,
          note: d.description ?? ''
        })),
  ...
  for (const inf of file.Inflections ?? []) {
    ...
    project.docs.push(
      page(lang.id, `屈折表 · ${title}`, (inf.categories ? `${inf.categories}\n\n` : '') + md)
    )
    report.docs++
  }

  if (file.IgnoreDiacritics === false || file.CaseSensitive)
    report.warnings.push('Lexicanter 的大小写 / 变音符敏感设置未迁移')
  project.meta.updatedAt = now()
  return report
}

function pronToRecord(
  p: LexcPron | undefined,
  mainLect: string,
  orthoId: Id
...
function wordToLexeme(
  lang: Language,
  word: string,
  entry: LexcWord,
  defLang: string,
  dialectIds: Map<string, Id>,
  mainLect: string,
  orthoId: Id
): Lexeme {
  ...
  // 其他方言的发音不丢：写进备注
  const extra = Object.entries(entry.pronunciations ?? {}).filter(
    ([l, v]) => l !== mainLect && v.ipa
  )
  if (extra.length)
    lx.notes = extra
      .map(([l, v]) => `发音（${l}）：${v.ipa}${v.irregular ? '（不规则）' : ''}`)
      .join('\n')
  return lx
}
      ...
      case 'list': {
        ...
        out.push(
          items
            .map(
              (it, i) =>
                `${ordered ? `${i + 1}.` : '-'} ${strip(typeof it === 'object' && it ? (it as { content?: string }).content : it)}`
            )
            .join('\n')
        )
        break
