...
export function buildIndex(project: Project, languageId: Id): GlossIndex {
  ...
  const idx: GlossIndex = { lemma: new Map(), stems: new Map(), forms: new Map(), morphemes: new Map(), prefixes: [], suffixes: [], confirmed: new Map(), glossLangs }
  for (const l of project.lexemes) {
    ...
    if (para) for (const s of paradigmSlots(para, project.categories, glossLangs)) abbrs.set(s.label, s.abbr)
...
const key = (a: Analysis): string => JSON.stringify([a.lexemeId, a.slot, a.morphs.map((m) => [m.form, m.gloss, m.morphemeId])])

/** 一个完整词（不含内部边界）的候选：词头 / 屈折形 / 词干 / 语素 */
function wholeWord(idx: GlossIndex, w: string): Analysis[] {
  const out: Analysis[] = []
  for (const l of idx.lemma.get(w) ?? []) out.push({ lexemeId: l.id, slot: null, morphs: [{ form: w, gloss: lexemeGloss(l, idx.glossLangs), morphemeId: null }] })
  for (const f of idx.forms.get(w) ?? []) out.push({ lexemeId: f.lexeme.id, slot: f.slot, morphs: [{ form: w, gloss: `${lexemeGloss(f.lexeme, idx.glossLangs)}.${f.abbr}`, morphemeId: null }] })
  for (const l of idx.stems.get(w) ?? []) out.push({ lexemeId: l.id, slot: null, morphs: [{ form: w, gloss: lexemeGloss(l, idx.glossLangs), morphemeId: null }] })
  for (const m of idx.morphemes.get(w) ?? []) out.push({ lexemeId: null, slot: null, morphs: [{ form: w, gloss: morphemeGloss(m, idx.glossLangs), morphemeId: m.id }] })
  return out
}
...
function stripAffixes(idx: GlossIndex, w: string, depth: number): Analysis[] {
  ...
  for (const s of idx.suffixes) {
    if (w.length > s.form.length && w.endsWith(s.form)) {
      ...
      for (const inner of stripAffixes(idx, rest, depth - 1)) {
        out.push({ ...inner, morphs: [...inner.morphs, { form: s.form, gloss: morphemeGloss(s.morpheme, idx.glossLangs), morphemeId: s.morpheme.id }] })
      }
    }
  }
  for (const p of idx.prefixes) {
    if (w.length > p.form.length && w.startsWith(p.form)) {
      ...
      for (const inner of stripAffixes(idx, rest, depth - 1)) {
        out.push({ ...inner, morphs: [{ form: p.form, gloss: morphemeGloss(p.morpheme, idx.glossLangs), morphemeId: p.morpheme.id }, ...inner.morphs] })
      }
    }
  }
  return out
...
export function analyzeSentence(project: Project, sentence: Sentence, opts: { force?: boolean } = {}): Sentence {
...
export function coverage(s: Sentence): { total: number; resolved: number; confirmed: number } {
  ...
  const resolved = s.tokens.filter((t) => t.analyses[t.chosen] && !t.analyses[t.chosen].morphs.some((m) => m.gloss === '?')).length
...
export function renderTemplate(template: string, il: Interlinear, s: Sentence): string {
  ...
  const tokensBlock = template.replace(/\{\{#tokens\}\}([\s\S]*?)\{\{\/tokens\}\}/g, (_m, inner: string) =>
    il.words.map((w, i) => fill(inner, { surface: w.surface, morphs: w.morphs, gloss: w.gloss, sep: i ? ' ' : '' })).join('')
  )
...
export function corpusStats(project: Project, languageId: Id): { frequency: { surface: string; n: number }[]; unresolved: string[]; lexemeCoverage: number } {
  ...
  return {
    frequency: [...freq.entries()].map(([surface, n]) => ({ surface, n })).sort((a, b) => b.n - a.n),
...
export const LEIPZIG: { abbr: string; en: string; zh: string }[] = [
  ['1', 'first person', '第一人称'], ['2', 'second person', '第二人称'], ['3', 'third person', '第三人称'], ['A', 'agent-like argument', '施事论元'], ['ABL', 'ablative', '夺格'], ['ABS', 'absolutive', '通格'], ['ACC', 'accusative', '宾格'], ['ADJ', 'adjective', '形容词'], ['ADV', 'adverb(ial)', '副词'], ['AGR', 'agreement', '一致'], ['ALL', 'allative', '向格'], ['ANTIP', 'antipassive', '反被动'], ['APPL', 'applicative', '施用'], ['ART', 'article', '冠词'], ['AUX', 'auxiliary', '助动词'], ['BEN', 'benefactive', '受益格'], ['CAUS', 'causative', '使役'], ['CLF', 'classifier', '量词'], ['COM', 'comitative', '伴随格'], ['COMP', 'complementizer', '补语标记'], ['COMPL', 'completive', '完结体'], ['COND', 'conditional', '条件'], ['COP', 'copula', '系词'], ['CVB', 'converb', '副动词'], ['DAT', 'dative', '与格'], ['DECL', 'declarative', '陈述'], ['DEF', 'definite', '定指'], ['DEM', 'demonstrative', '指示词'], ['DET', 'determiner', '限定词'], ['DIST', 'distal', '远指'], ['DISTR', 'distributive', '分配'], ['DU', 'dual', '双数'], ['DUR', 'durative', '持续'], ['ERG', 'ergative', '作格'], ['EXCL', 'exclusive', '排除式'], ['F', 'feminine', '阴性'], ['FOC', 'focus', '焦点'], ['FUT', 'future', '将来'], ['GEN', 'genitive', '属格'], ['IMP', 'imperative', '祈使'], ['INCL', 'inclusive', '包括式'], ['IND', 'indicative', '直陈'], ['INDF', 'indefinite', '不定指'], ['INF', 'infinitive', '不定式'], ['INS', 'instrumental', '工具格'], ['INTR', 'intransitive', '不及物'], ['IPFV', 'imperfective', '未完成体'], ['IRR', 'irrealis', '非现实'], ['LOC', 'locative', '位格'], ['M', 'masculine', '阳性'], ['N', 'neuter', '中性'], ['NEG', 'negation', '否定'], ['NMLZ', 'nominalizer', '名物化'], ['NOM', 'nominative', '主格'], ['OBJ', 'object', '宾语'], ['OBL', 'oblique', '斜格'], ['P', 'patient-like argument', '受事论元'], ['PASS', 'passive', '被动'], ['PFV', 'perfective', '完成体'], ['PL', 'plural', '复数'], ['POSS', 'possessive', '领属'], ['PRED', 'predicative', '谓语'], ['PRF', 'perfect', '完成时'], ['PRS', 'present', '现在'], ['PROG', 'progressive', '进行'], ['PROH', 'prohibitive', '禁止'], ['PROX', 'proximal', '近指'], ['PST', 'past', '过去'], ['PTCP', 'participle', '分词'], ['PURP', 'purposive', '目的'], ['Q', 'question particle', '疑问'], ['QUOT', 'quotative', '引语'], ['RECP', 'reciprocal', '相互'], ['REFL', 'reflexive', '反身'], ['REL', 'relative', '关系'], ['RES', 'resultative', '结果'], ['S', 'single argument', '单论元'], ['SBJ', 'subject', '主语'], ['SBJV', 'subjunctive', '虚拟'], ['SG', 'singular', '单数'], ['TOP', 'topic', '话题'], ['TR', 'transitive', '及物'], ['VOC', 'vocative', '呼格']
