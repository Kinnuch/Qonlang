...
export function defaultFeatures(symbol: string): Record<string, string> {
  ...
  for (let hi = 0; hi < VOWELS.length; hi++) {
    for (let bi = 0; bi < VOWELS[hi].length; bi++) {
      ...
      if (cell[0] === base)
        return {
          type: 'vowel',
          height: HEIGHTS[hi].en,
          backness: BACKNESS[bi].en,
          round: 'no',
          syllabic: 'yes'
        }
      if (cell[1] === base)
  ...
  for (const g of NON_PULMONIC)
    if (g.items.some((v) => v.s === base))
      return { type: 'consonant', manner: g.en.toLowerCase(), syllabic: 'no' }
  // 常见附加特征
