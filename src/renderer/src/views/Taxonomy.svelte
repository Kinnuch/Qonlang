<script lang="ts">
  ...
  function catUse(c: GrammaticalCategory): number {
    return project.lexemes.filter((l) => c.id in l.features).length + project.morphemes.filter((m) => c.id in m.features).length
  }
...
<div class="tax">
  <section>
    ...
    {#each project.posList as p (p.id)}
      <div class="card item">
        <div class="grow">
          <LocalizedInput bind:value={p.name} languages={glossLangs} placeholder={t('taxonomy.posName')} onchange={() => projectState.touch()} />
        </div>
        <div class="field abbr">
          <label for={`pos-abbr-${p.id}`}>{t('taxonomy.abbr')}</label>
          <input id={`pos-abbr-${p.id}`} class="input mono" bind:value={p.abbr} oninput={() => projectState.touch()} />
        </div>
        <span class="small muted use">{t('taxonomy.inUse', { n: posUse(p) })}</span>
        <button class="btn ghost icon sm danger" title={t('common.delete')} onclick={() => removePos(p)}><Trash2 size={14} /></button>
      </div>
    {/each}
  </section>
  ...
  <section>
    <div class="row head">
      ...
      <button class="btn sm" onclick={addCategory}><Plus size={14} />{t('taxonomy.addCategory')}</button>
    </div>
    ...
    {#each project.categories as c (c.id)}
      <div class="card cat">
        <div class="row">
          <div class="grow">
            <LocalizedInput bind:value={c.name} languages={glossLangs} placeholder={t('taxonomy.categoryName')} onchange={() => projectState.touch()} />
          </div>
          <span class="small muted use">{t('taxonomy.inUse', { n: catUse(c) })}</span>
          <button class="btn ghost icon sm danger" title={t('common.delete')} onclick={() => removeCategory(c)}><Trash2 size={14} /></button>
        </div>
        <div class="values">
          ...
          {#each c.values as v, i (v.id)}
            <div class="row val">
              {#each glossLangs as lg (lg)}
                <input class="input" placeholder={`${t('taxonomy.valueName')} (${lg})`} bind:value={v.name[lg]} oninput={() => projectState.touch()} />
              {/each}
              <input class="input mono abbr-in" placeholder={t('taxonomy.valueAbbr')} bind:value={v.abbr} oninput={() => projectState.touch()} />
              <button class="btn ghost icon sm" onclick={() => { c.values.splice(i, 1); projectState.touch() }}><X size={14} /></button>
            </div>
          {/each}
          <button class="btn ghost sm self-start" onclick={() => { c.values.push({ id: newId(), name: {}, abbr: '' }); projectState.touch() }}><Plus size={14} />{t('taxonomy.addValue')}</button>
        </div>
      </div>
    {/each}
  </section>
